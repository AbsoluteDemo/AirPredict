...
import service.history_data as history_data
...
# 获取系统版本
@app.route('/predict/show', methods=["POST"])
def predict_show():
    res = version_data.get_predict_version()
    return jsonify({"data": res})
...
@app.route('/map/total')
def get_city_data():
    return view_data.get_AQI_city_data()
...
def history_list():
    ...
    page_no = get_data.get('page_no')
    param = get_data.get('param')
    data, count, page_list, max_page = history_data.get_weathers_list(int(page_size), int(page_no), param)
    return jsonify({"data": data, "count": count, "page_no": page_no, "page_list": page_list, "max_page": max_page})
...
def history_old_edit():
    get_data = request.form.to_dict()
    collectdata = get_data.get('collectdata')
    aqi_lv = get_data.get('aqi_lv')
    aqi = get_data.get('aqi')
    PM25 = get_data.get('PM25')
    PM10 = get_data.get('PM10')
    So2 = get_data.get('So2')
    No2 = get_data.get('No2')
    Co = get_data.get('Co')
    O3 = get_data.get('O3')
    history_data.edit_weathers(collectdata,aqi_lv , aqi, PM25, PM10, So2, No2,Co,O3)
    return '200'


# 修改天气数据
@app.route('/history/del', methods=["PUT"])
def history_old_del():
    get_data = request.form.to_dict()
    id = get_data.get('collectdata')
    history_data.del_weathers(id)
    return '200'
