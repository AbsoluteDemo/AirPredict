...
def get_AQI_city_data():
    ...
    db.close_commit()
    return jsonify({"lh":lh,"zd":zd,"pc":pc,"c":c,"jc":jc,"yd":yd})


# 获取城市空气质量统计
def get_AQI_hangzhou_data():
    db = dbUtil()
    lh_sql = "SELECT COUNT(collectdata) FROM AQIData WHERE aqi<=50"
    zd_sql = "SELECT COUNT(collectdata) FROM AQIData WHERE aqi>50 AND aqi<=100"
    pc_sql = "SELECT COUNT(collectdata) FROM AQIData WHERE aqi>100 AND aqi<=150"
    c_sql = "SELECT COUNT(collectdata) FROM AQIData WHERE aqi>150 AND aqi<=200"
    jc_sql = "SELECT COUNT(collectdata) FROM AQIData WHERE aqi>200 AND aqi<=300"
    yd_sql = "SELECT COUNT(collectdata) FROM AQIData WHERE aqi>300"
    lh = db.query_noargs(lh_sql)[0][0]
    zd = db.query_noargs(zd_sql)[0][0]
    pc = db.query_noargs(pc_sql)[0][0]
    c = db.query_noargs(c_sql)[0][0]
    ...
    yd = db.query_noargs(yd_sql)[0][0]
    db.close_commit()
    return jsonify([{"time": "优", "value": lh, "name": "空气质量"},
                    {"time": "良", "value": zd, "name": "空气质量"},
                    {"time": "轻度污染", "value": pc, "name": "空气质量"},
                    {"time": "中度污染", "value": c, "name": "空气质量"},
                    {"time": "重度污染", "value": jc, "name": "空气质量"},
                    {"time": "严重污染", "value": yd, "name": "空气质量"}])
...
def get_bg_total_data():
    db = dbUtil()
    bg_sql = "SELECT record_time,temp, wse FROM weather WHERE cityname='杭州' AND  to_days(create_time) = to_days(now());"
